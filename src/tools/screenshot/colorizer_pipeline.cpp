...
std::string compute_config_hash(const std::string& theme_name,
    ...
    static_assert(sizeof(ColorizerDisplayConfig) == 20,
...
// pre-work baseline. Takes a pre-taken sample so callers can measure before any
// bench-only re-runs that would inflate the numbers.
...
// PNG save epilogue for the render path: ensure the output directory exists
// and write the bitmap. Returns `out` on success, empty on failure.
...
std::wstring run_colorizer_pipeline(const Options& opts) {
    ...
    // attributable to opening this file (factories + colorize + layout).
    ...
    // so the tool resolves the same colors as the plugin regardless of CWD.
    ...
    std::string lang = base_lang;
    if (!colorizer.supports(lang)) {
        std::fprintf(stderr,
            "No Lexilla lexer registered for \"%s\"\n", lang.c_str());
        return {};
    }
    ...
    auto _tcolor0 = std::chrono::steady_clock::now();
    ...
    if (opts.bench) {
        ...
        std::fprintf(stderr, "    colorize (lex + map) %8.2f ms\n", ctimings.highlight_ms);
        std::fprintf(stderr, "  colorize (warm)  %8.2f ms\n", ms(_w0, _w1));
        std::fprintf(stderr, "    colorize (lex + map) %8.2f ms\n", cwarm.highlight_ms);
