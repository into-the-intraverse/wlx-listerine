...
- **Search match counter for the colorizer plugin** — port the markdown plugin's bottom-right `current / total` HUD (see `src/search_hud*.{h,cpp}`) to the colorizer (`src/colorizer/colorizer_host_adapter.cpp`). Mostly a copy of the markdown integration plus the same window-class registration in `DllMain`.
- **DPI scaling for the document render target** — `RenderEngine::create_device_resources` (and `SearchHud::recreate_target` until they're aligned) creates the D2D HwndRenderTarget at the default 96 DPI, so on Per-Monitor V2 displays at 125%+ scaling the document text and chrome render at physical-pixel size and look small. Plumb `GetDpiForWindow(hwnd)` through both creation paths.
