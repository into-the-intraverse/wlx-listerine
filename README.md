...
- **DPI scaling for the document render target** — `RenderEngine::create_device_resources` creates the D2D HwndRenderTarget at the default 96 DPI, so on Per-Monitor V2 displays at 125%+ scaling the document text and chrome render at physical-pixel size and look small. `SearchHud::recreate_target` already plumbs `GetDpiForWindow(hwnd)` through; mirror that here.
